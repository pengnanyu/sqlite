let SQL=null,localDb=null,currentDb=null,currentTable=null,dbModified=false;
...
async function api(path,options={}){
  ...
  if(opts.body&&typeof opts.body!=='string'){
    ...
    else{opts.headers['Content-Type']='application/json';opts.body=JSON.stringify(opts.body)}
  }
  ...
  if(ct.includes('octet-stream')){if(!res.ok)throw new Error(`HTTP ${res.status}`);return await res.arrayBuffer()}
...
async function syncToServer(){
  ...
  if(u8.length>10*1024*1024)throw new Error(`数据库文件过大(${(u8.length/1024/1024).toFixed(1)}MB)，KV限制约25MB`);
  const formData=new FormData();
  formData.append('file',new Blob([u8],{type:'application/octet-stream'}),`${currentDb}.db`);
  const t=queryLocal("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'");
  formData.append('tables',JSON.stringify(t.map(r=>r.name)));
  await api(`/databases/${encodeURIComponent(currentDb)}/save`,{method:'POST',body:formData});
  dbModified=false;
}
